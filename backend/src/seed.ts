...
async function seed() {
  await mongoose.connect(process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/food_ordering_app");
  await Promise.all([User.deleteMany({}), Restaurant.deleteMany({}), Category.deleteMany({}), Product.deleteMany({})]);

  const passwordHash = await bcrypt.hash("Admin123!", 12);
  await User.create({ name: "Admin", email: "admin@foodapp.local", password: passwordHash, role: "admin" });
  await User.create({ name: "Demo User", email: "user@foodapp.local", password: await bcrypt.hash("User123!", 12), role: "user" });
  ...
  const [burgers, pizza, pasta, healthy] = await Category.create([
